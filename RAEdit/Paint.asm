...
SetBlockMarkers proc uses ebx esi edi,hMem:DWORD,nLine:DWORD,nMax:DWORD
	...
	mov		ebx,hMem
	.if [ebx].EDIT.fstyleex&STYLEEX_BLOCKGUIDE
		mov		fcmnt,0
		;Clear block markers
...
NestedProc_BlockRoot:
	...
	.if eax!=-1
		...
		.if eax!=-1
			...
			mov		edx,lpBlockDef
			.if !([edx].RABLOCKDEF.flag&BD_INCLUDELAST)
				inc		eax
			.endif
			add		eax,esi
			.if eax>=nLine
...
DrawChangedState proc uses ebx edi,hMem:DWORD,hDC:HDC,lpLine:DWORD,x:DWORD,y:DWORD
	...
	mov		ebx,hMem
	.if [ebx].EDIT.fstyleex&STILEEX_LINECHANGED
		mov		edi,lpLine
		.if [edi].CHARS.state&STATE_CHANGESAVED
		...
		.else
			.if [edi].CHARS.state&STATE_CHANGED
...
RAEditPaint proc uses ebx esi edi,hWin:HWND
	...
	.if [ebx].EDIT.linenrwt
		.if [ebx].EDIT.fstyle&STYLE_AUTOSIZELINENUM
			xor		eax,eax
			mov		rect1.left,eax
			mov		rect1.top,eax
			mov		rect1.right,eax
...
;				invoke SetCaret,ebx,[esi].RAEDT.cpy
	...
	.if !([ebx].EDIT.fstyle&STYLE_NOVSCROLL)
		mov		eax,SBWT
		sub		rect.right,eax
	.endif
	mov		eax,rect.right
	  ...
	  @@:
		...
		.if !([edi].CHARS.state&STATE_HIDDEN)
			...
			.if eax>ps.rcPaint.top
				...
				.if [edi].CHARS.state&STATE_DIVIDERLINE
					.if !([ebx].EDIT.fstyle&STYLE_NODIVIDERLINE)
...
NestedProc_DrawBlockMarker:
	...
	.endif
	.if [edi].CHARS.state&STATE_BLOCKEND
		xor		eax,eax
		sub		eax,ps.rcPaint.left
		add		eax,[ebx].EDIT.linenrwt
		add		eax,15
